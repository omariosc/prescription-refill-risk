...
function P2ULogoLarge() {
  return (
    <svg style={{ height: 48, width: 'auto', display: 'block', margin: '0 auto' }} viewBox="0 0 1226.57 302.38" xmlns="http://www.w3.org/2000/svg">
...
export default function PatientDesktopGate({ onContinue, onLogout }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/magic-link', {
      ...
      .then(data => setMagicUrl(data?.url || baseUrl))
      .catch(() => setMagicUrl(baseUrl));
  }, []);

  useEffect(() => {
    if (!magicUrl || !qrRef.current || !window.QrCreator) return;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          The Pharmacy2U patient app is designed for mobile devices. Scan the QR code to open and log in automatically.
        </p>
        ...
        <div style={{
          ...
          {magicUrl || 'Generating secure link…'}
        </div>
